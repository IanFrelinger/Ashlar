...
- [x] **Marketing landing honest:** check actually performed 2026-09-15, claim by claim against the code rather than assumed. Cloud is marked "Coming soon"; no autonomous-production or Forge-GA claim appears. Every technical claim verified: `MapPost("/copilot/task")` and `MapGet("/trust/dashboard")` exist, the trust log does carry `SourceId`, `AddAshlarClient(baseUrl, apiKey?)` and `InvokeAsync(HttpMethod, string, HttpContent?, CancellationToken)` match the samples' shapes, `DoctorCommand` backs `ashlar doctor`, `Ashlar.Client` is on nuget.org, Ollama is the default route, the licence is Apache 2.0, the five required checks the page names are the five that are actually required, and TesterQuickstart really does say fifteen minutes with no Docker and no API keys. **One claim was false and is fixed in the same pass:** the pricing table sold SSO with a tick on all three paid tiers and no SAML/OIDC/OpenID exists in `src`, `application`, `commercial` or `products` — split into an earned "Audit export" row and an "SSO / SAML" row marked Roadmap. The terminal-preview SVG also read `v0.7.0` against a VERSION of 0.2.0
