...
| `kernel-gate.yml` | Kernel Gate / `kernel-gate` | paths: `src/Ashlar.Hosting/**`, Abstractions, AI Pipeline source/tests, Infrastructure, Orchestration, Runtime, Core.Application, kernel tests, `docs/production-readiness/**`, `Makefile`, shared build inputs | push (narrower paths, including AI source/tests and shared build inputs), dispatch |
...
also runs that suite on net8.0 through `ci verify` → `validate`; [master run 34721291876](https://github.com/IanFrelinger/Ashlar/actions/runs/34721291876)
recorded 62 passed tests and a project-specific TRX receipt on each of Linux, macOS and Windows.
That count is an observation, not a permanent test floor. Both AI directory trees now select both
routes. `build-core` already compiles the AI library through Hosting/CLI, and is unchanged.
`AiPipelineCiRoutingConventionTests` protects nested-path eligibility, shared inputs, readiness list
parity and the direct Kernel Make edge. To observe routing, inspect a PR whose diff contains only
an AI source/test edit: Kernel must start automatically, readiness must print the matched AI path
and `run_heavy=true`, and native jobs must produce AI suite receipts. A workflow-file edit or manual
dispatch proves execution, but cannot prove that an AI-only diff selected it.
