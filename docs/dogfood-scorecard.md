...
- ✅ lim-9 closed 2026-09-13: explicit `hmacKey` is honoured; no operator path to a real key for compositions: the injected brick signer is still discarded, `CertificationRecordSigner` exposes no key accessor, and no production registration supplies the `hmacKey` parameter
