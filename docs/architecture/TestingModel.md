...
Today `master` branch protection requires **five** checks: **`cert-gate`**, **`build-core`**, **`shell-lint`**, **`lychee (README + docs)`**, and **`Readiness summary`** (see [CI gate inventory](../CiGateInventory.md)); the coverage gates run on PRs that touch kernel paths but do not block merges. To block merges when kernel line coverage regresses, in GitHub go to **Settings → Branches → Branch protection rule**, enable **Require status checks to pass**, and add **`kernel-coverage`** (composite gate as enforced by `scripts/ci/kernel-coverage-gate.sh`: Domain 100%, Infrastructure 80%, Core.Application 67%; the former `domain-coverage` workflow was folded into it on 2026-08-16). The workflow is path-filtered, so give it an always-report job first or the required context will never appear on PRs outside those paths. See [Coverage gates v1](../production-readiness/CoverageGates-v1.md). If the UI only shows the workflow name, pick the check that corresponds to each workflow’s latest green run.
