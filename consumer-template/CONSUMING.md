...
|----------|------------|------------------------|
| `net8.0`, `net10.0` | Everything: brick authoring and hosting, SDK/client, HMAC **and** Ed25519 signing/verification, `CertificationVerifyOptions.Default` / `.Strict` / pinned verdicts. | — `net8.0` is the **floor for trust**: Ed25519 lives in `CertificationRecordEd25519`, which is compiled only under `NET8_0_OR_GREATER` because `NSec.Cryptography` 25.4.0 ships `lib/net8.0` only, and `Default` and `Strict` both set `RequireEd25519Signature`. |
...
| `Ashlar.Certification.State` | `netstandard2.0;net8.0;net10.0` |
...
`Ashlar.Certification.Contracts` and `Ashlar.Certification.State` are not in the pin list above (nor in `scripts/consumer-surface-packages.txt`): the contracts arrive transitively through `Ashlar.Hosting.Bundle`; a verify-only consumer, or one that needs the state log, pins them directly at the same version.
