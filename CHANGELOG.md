...
- **A stale vector of the wrong dimension is now skipped by `SqliteVectorStore.SearchAsync` instead of being returned as a hit.** `VectorMath.CosineSimilarity` returns 0 when the two vectors differ in length, and the search admitted every row scoring at or above `minScore` — so at the common `minScore` of 0.0, a row left behind by a differently-dimensioned generator came back as a score-0.0 result rather than being ignored. Rows whose blob length does not match the query's dimension are dropped before scoring.
