...
Development is trunk-based: `master` is the only long-lived branch. Branch protection requires **five** status checks: `cert-gate`, `build-core`, `shell-lint`, `lychee (README + docs)`, and `Readiness summary` (plus "up to date with base"); every other workflow is advisory — see [`docs/CiGateInventory.md`](docs/CiGateInventory.md) and "Layer boundary and what master actually enforces" below.

These five checks run on every PR and always report a status, so they are safe as required checks and will not block PRs that don't touch their relevant code paths (the first four have no path filter; `Readiness summary` filters paths inside its workflow via a `changes` job and passes in ~1 min when no core path changed).
